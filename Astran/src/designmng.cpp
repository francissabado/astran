...
bool DesignMng::readCommand(string cmd){
        try {
                ...
                if (words.size() != 0) {
                        ...
                        /****  EXPORT - 3  ****/
                        // {"EXPORT LAYOUT <str_Layout> <str_Path>", "Exports layout generated by ICPD to open with other applications.
                        // Supported extensions: .cif (Calltech Intermediate Format), .gds (Graphic Data System II), .c2c (CIF to Cadence Convertion File)"},
                        case EXPORT_LAYOUT: {
                                string str_Layout = words[2];
                                string str_Path = words[3];
                                exportLayout(str_Layout, str_Path);
                        }
                        break;


                        // {"EXPORT LAYOUTS <str_Path>", "Export all layouts to a directory using .cif and .gds formats."},
                        case EXPORT_LAYOUTS: {
                                string str_Path = words[2];
                                exportLayouts(str_Path);
                        } break;
...
//Recreate commands as functions
...
void DesignMng::exportLayout(string str_Layout, string str_Path){

        string tipo = upcase(getExt(str_Path));
        string filename = str_Path;
        // string cellName = "";
        // size_t atmp_Pos = str_Layout.find("_atmp_");
        // if( atmp_Pos != -1 ) {
        //         cellName = str_Layout.substr(0, atmp_Pos);
        // }

        // if(circuit->getLayout(upcase(cellName))) {

        if(circuit->getLayout(str_Layout)) {
                if(tipo=="GDS") {
                        rules->saveGDSIILayerTable(getPath(filename)+"GDSIILTable.txt");
                        cout << "-> Saving layout " << str_Layout << " to file: " << filename << endl;
                        Gds g(filename);
                        char tmp[20] = "                   ";
                        strcpy(tmp, str_Layout.substr(0,19).c_str());
                        g.open();
                        g.generateHeader(1);
                        g.generateLibrary();
                        ...
                        for (layers_it = circuit->getLayout(str_Layout)->layers.begin(); layers_it != circuit->getLayout(str_Layout)->layers.end(); layers_it++) {
                        ...
                        for (labels_it = circuit->getLayout(str_Layout)->labels.begin(); labels_it != circuit->getLayout(str_Layout)->labels.end(); labels_it++) {
                        ...
                        cifOut.cellCif(*(circuit->getLayouts()), str_Layout);
                        cifOut.cif2Cadence(name,  str_Layout);
                }else if(tipo=="CIF") {
                ...
                throw AstranError("Cell not found: " + str_Layout);

}

// {"EXPORT LAYOUTS <str_Path>", "Export all layouts to a directory using .cif and .gds formats."},
void DesignMng::exportLayouts(string str_Path){
        ...
        {
                string layoutName = it->first;
                string filePath = str_Path + "/" + it->first;

                //Print GDS
                exportLayout(layoutName, filePath + ".gds");
                exportLayout(layoutName, filePath + ".c2c");
        }

}


void DesignMng::listAllLayouts(){
        map<string, CLayout>* allLayouts = circuit->getLayouts();
